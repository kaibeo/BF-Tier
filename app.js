...
function initShootingStars() {
  ...
  function loop() {
    ...
    meteors = meteors.filter((m) => m.life > 0 && m.y < canvas.height + 100);
...
function animateHeroStats() {
  const playersEl = document.getElementById('heroTotalPlayers');
  const modesEl = document.getElementById('heroTotalModes');
  const eloEl = document.getElementById('heroAvgElo');

  if (!playersEl || !modesEl || !eloEl) return;

  const targetPlayers = PLAYERS.length;
  const targetModes = 9;
  const targetElo = Math.max(...PLAYERS.map(p => p.stats.elo || 0));

  animateCount(playersEl, targetPlayers, 1500);
  animateCount(modesEl, targetModes, 1000);
  animateCount(eloEl, targetElo, 1500);
}
